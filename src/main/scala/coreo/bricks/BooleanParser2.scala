...
object BoolSimplify:
  def simplify(e: BoolExpr): BoolExpr = e match
    ...
    case And(a, b) =>
      val sa = simplify(a); val sb = simplify(b)
...
object BoolParser:
  ...
  def tokenize(input: String): List[Tok] =
    ...
    def next(): Char = { val c = peek; i += 1; c }
  ...
  final class Parser(tokens: List[Tok]):
    ...
    private def parseOrNor(): BoolExpr =
      ...
      while loop do
        cur match
          case TOr  => eat(TOr);  val r = parseXorNxor(); left = Or(left, r)
          case TNor => eat(TNor); val r = parseXorNxor(); left = Nor(left, r)
          case _    => loop = false
      left
    ...
    private def parseXorNxor(): BoolExpr =
      ...
      while loop do
        cur match
          case TXor  => eat(TXor);  val r = parseAndNand(); left = Xor(left, r)
          case TNxor => eat(TNxor); val r = parseAndNand(); left = Nxor(left, r)
          case _     => loop = false
      left
    ...
    private def parseAndNand(): BoolExpr =
      ...
      while loop do
        cur match
          case TAnd  => eat(TAnd);  val r = parseNot(); left = And(left, r)
          case TNand => eat(TNand); val r = parseNot(); left = Nand(left, r)
          case _     => loop = false
      left
    ...
    private def parsePrimary(): BoolExpr = cur match
      case TLParen   => eat(TLParen); val e = parseExpr(); eat(TRParen); e
